...
$get_metoda = $this->db_master->get('master_metoda_obat')->result();
?>
		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				<div class="panel-heading text-right">
					<span class="pull-left" style="font-size: 24px">Daftar Metoda Obat</span>
					...
					<div class="col-md-12 row">
						...
						<table class="table table-striped table-hover table-bordered" id="datatable"  style="font-size:1.0em;">
							...
							<tbody>
								<?php 
								$no = 0;
								foreach ($get_metoda as $value) { 
									$no++; ?>
									<tr>
										<td><?= $no ?></td>
										<td><?= $value->kode_metoda_obat ?></td>
										<td><?= $value->metoda_obat ?></td>
									...
									<td>
										<div class="btn-group">
											<a href="<?php echo base_url('master/metoda_obat/edit/'.$value->id)?>" id="edit" data-toggle="tooltip" title="Edit" class="btn btn-icon-only btn-circle btn-warning"><i class="fa fa-pencil"></i></a>
...
<script type="text/javascript">
	...
	function hapus_data() {
		var id=$('#id').val();
		$.ajax({
			url: '<?php echo base_url('master/metoda_obat/hapus_metoda'); ?>',
			type: 'post',
			data:{id:id},
			beforeSend:function(){
				$(".tunggu").show();
