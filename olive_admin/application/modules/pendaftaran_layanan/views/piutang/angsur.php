		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				<div class="panel-heading text-right">
					<span class="pull-left" style="font-size: 24px">Angsuran Piutang </span>
					<a href="<?php echo base_url('penjualan/piutang/detail/'.@$kode_member); ?>" class="btn btn-primary btn-no-radius"><i class="fa fa-list"></i> Data Piutang</a>
				</div>
				<?php
...
<script type="text/javascript">
	function get_jenis_angsuran(){
		...
		if(jenis_angsuran=='lunas'){
			$('#dibayar').val(sisa_piutang);
			$('#dibayar').attr('readonly',true);
			$('#text_dibayar').html(toRp(sisa_piutang));
			$('.jatuh_tempo').hide();
		}else{
			$('#dibayar').val(0);
	...
	function cek_dibayar(){
		...
		var sisa_piutang=$('#sisa_piutang').val();
		if(parseInt(dibayar) > parseInt(sisa_piutang)){
	...
	function simpan_angsuran(){
		...
		if (jenis_angsuran != '' || dibayar != '' || kode_member != '' || jenis_pembayaran != '') {
			$.ajax({
				url: '<?php echo base_url('penjualan/piutang/simpan_angsuran'); ?>',
				type: 'post',
				data:{  kode_transaksi:kode_transaksi,
					...
					sisa_piutang:sisa_piutang,tanggal_jatuh_tempo:tanggal_jatuh_tempo},
					beforeSend:function(){
						$(".tunggu").show();
					},
					success: function(hasil){
						...
						window.location="<?php echo base_url('penjualan/piutang/detail/'.@$kode_member); ?>";
					},
				});			
		}else{
			alert('Lengkapi Form ...!');
