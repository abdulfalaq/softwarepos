						...
						<form id="data_form" onsubmit="return false;">
							...
							<table id="" class="table table-striped table-bordered">
								...
								foreach ($hasil_opsi_pengadaan as $opsi) {
									...
									$subtotal  += $kekurangan * $opsi->hpp;
									?>
									<tbody  hidden="">
										<tr id="tr_supplier<?php echo $opsi->kode_bahan_baku?>" class="tr_dupliacte">
											<td></td>
											<td></td>
											<td>
												<input type="hidden" id="kode_bahan" value="<?php echo $opsi->kode_bahan_baku;?>">
												<select name="kode_supplier_<?php echo $opsi->kode_bahan_baku?>[]" id='kode_supplier' class="form-control select_supp" >
											...
											<td></td>
											<td>
												<input type="number" name="qty_po_<?php echo $opsi->kode_bahan_baku?>[]" id="qty_po"  class="form-control hitung_subtotal qty_po_<?php echo $opsi->kode_bahan_baku?>" value="" onkeyup="hitung_subtotal(this)">
											</td>
											<td>
												<input type="number" name="harga_<?php echo $opsi->kode_bahan_baku?>[]" id="harga_satuan" class="form-control hitung_subtotal" value="<?php echo @$opsi->hpp;?>" onkeyup="hitung_subtotal(this)">
											</td>
											<td class="text-right">
												<label id="label_jml_harga"></label>
												<input type="hidden" name="jml_harga_<?php echo $opsi->kode_bahan_baku;?>[]" id="jml_harga" value="" class="subtotal">
											</td>
											<td class="text-center">
												<button class="btn btn-danger btn_add" onclick="delete_supplier(this)" key=""><i class="fa fa-trash"></i></button>
											</td>
										</tr>
									</tbody>
									...
									<tbody id="copy_supplier<?php echo $opsi->kode_bahan_baku?>" >
										<tr>
											...
											<td>
												<select  name="kode_supplier_<?php echo $opsi->kode_bahan_baku?>[]" id='kode_supplier' class="form-control supplier select2" required="" >
													<option value="">- Pilih Supplier</option>
													<?php
													foreach ($hasil_supplier as $supplier) {
														?>
											...
											<td><?php echo @$kekurangan.' ';echo @$opsi->alias;?></td>
											<td>
												<input type="hidden" class="kekurangan_<?php echo $opsi->kode_bahan_baku?>" value="<?php echo @$kekurangan;?>">
												<input type="number" name="qty_po_<?php echo $opsi->kode_bahan_baku?>[]" id="qty_po" class="form-control  qty_po_<?php echo $opsi->kode_bahan_baku?>" value="<?php echo @$kekurangan;?>" onkeyup="hitung_subtotal(this)">
											</td>
											<td>
												<input type="number" name="harga_<?php echo $opsi->kode_bahan_baku?>[]" id="harga_satuan" class="form-control" value="<?php echo @$opsi->hpp;?>" onkeyup="hitung_subtotal(this)">
											</td>
											<td class="text-right">
												<label id="label_jml_harga" class="label_<?php echo $opsi->kode_bahan_baku;?>"><?php echo @format_rupiah($kekurangan * $opsi->hpp);?></label>
												<input type="hidden" name="jml_harga_<?php echo $opsi->kode_bahan_baku;?>[]"  id="jml_harga" value="<?php echo @$kekurangan * $opsi->hpp;?>" class="subtotal jml_harga_<?php echo $opsi->kode_bahan_baku;?>">
											</td>
											
											<td>
												<button class="btn btn-primary btn_add" onclick="tambah_supplier('<?php echo $opsi->kode_bahan_baku?>')" key="">Tambah Supplier</button>
											</td>
										</tr> 
									</tbody>
								...
								$nominal_ppn=($subtotal * 10)/100;
								$grandtotal=$subtotal+$nominal_ppn;
								?>
								<tr>
									<th colspan="7"></th>
									<th>Sub Total</th>
									<th class="text-right"><label id="label_subtotal"><?php echo @format_rupiah($subtotal);?></label></th> 
								...
								<tr>
									...
									<th>PPN 
										<div class="input-group">
											<input type="number" name='ppn' id="ppn" class="form-control" placeholder="PPN" aria-describedby="basic-addon1" value="10" onkeyup="hitung_grandtotal()">
											<span class="input-group-addon" id="basic-addon1">%</span>
										</div>
										
									</th>
									<th class="text-right"><label id="label_ppn"><?php echo @format_rupiah($nominal_ppn);?></label></th>
									<input type="hidden" name="nominal_ppn" id="nominal_ppn" value="<?php echo @$nominal_ppn;?>">    
									<td></td> 
								</tr>
								<tr>
									<th colspan="7"></th>
									<th>Grandtotal</th>
									<th class="text-right"><label id="label_grantotal"><?php echo @format_rupiah($grandtotal);?></label></th>
...
<script type="text/javascript">
	...
	function hitung_grandtotal(){
		...
		if(parseInt(ppn) < 0 || parseInt(ppn) > 100){
			...
			grandtotal= parseInt(subtotal) + parseInt(nominal_ppn);
