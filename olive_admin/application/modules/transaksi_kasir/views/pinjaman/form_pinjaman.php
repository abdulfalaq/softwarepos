				...
				<div class="panel-body">
					<div class="row">	
						<div class="col-lg-12">
							...
							<div class="input-group" style="width: 50%">
								<span class="input-group-addon" id="v_plafon">Rp.</span>
						...
						<div class="col-lg-12"><br>
							...
							<div class="input-group status_denda" style="width: 50%">
								<span class="input-group-addon" id="v_denda">Rp.</span>
							...
							<div class="input-group" style="width: 50%">
								<span class="input-group-addon" id="v_administrasi">Rp.</span>
								<input type="number" id="nominal_administrasi" name="nominal_administrasi" placeholder="0" class="form-control">
							</div><br>

							<label>Potongan</label>
							<div class="input-group" style="width: 50%">
								<select name="potongan" id="potongan" required="" onchange="status_angsuran()">
							...
							<div class="input-group" style="width: 50%">
								<select name="potongan_lain" id="potongan_lain" onchange="potongan_lain()">
									<option value="tidak">Tidak Ada</option>
									<option value="ada">Ada</option>					
								</select>
							</div><br>
							<label class="potongan_lain">Nominal Potongan Lain-lain</label>
							<div class="input-group potongan_lain" style="width: 50%">
								<span class="input-group-addon" id="v_potongan_lain">Rp.</span>
								<input type="number" id="nominal_potongan_lain" name="nominal_potongan_lain" placeholder="0" class="form-control">
							</div><br>
							<label>Potongan Dana Sosial</label>
							<div class="input-group" style="width: 50%">
								<select name="potongan_dansos" id="potongan_dansos" onchange="potongan_dansos()">
									<option value="tidak">Tidak Ada</option>
									<option value="ada">Ada</option>					
								</select>
							</div><br>
							<label class="potongan_dansos">Nominal Potongan Dana Sosial</label>
							<div class="input-group potongan_dansos" style="width: 50%">
								<span class="input-group-addon" id="v_potongan_dansos">Rp.</span>
								<input type="number" id="nominal_potongan_dansos" name="nominal_potongan_dansos" placeholder="0" class="form-control">
							</div><br>
						</div>
					</div>
				</div>
...
<script>
	...
	function potongan_lain() {
		var potongan_lain = $('#potongan_lain').val();
		if(potongan_lain == 'ada'){
			$('.potongan_lain').fadeIn("slow");
		}else{
			$('.potongan_lain').fadeOut("slow");
		}
	}
	function potongan_dansos() {
		var potongan_dansos = $('#potongan_dansos').val();
		if(potongan_dansos == 'ada'){
			$('.potongan_dansos').fadeIn("slow");
		}else{
			$('.potongan_dansos').fadeOut("slow");
		}
	}
	...
	$('#nominal_administrasi').keyup(function(){
		...
		if(nominal < 0){
			...
			$("#v_administrasi").text(toRp(nominal));
		}
	});
	...
	$('#nominal_denda').keyup(function(){
		...
		if(nominal < 0){
			...
			$("#v_denda").text(toRp(nominal));
		}
	});
	$('#nominal_potongan_lain').keyup(function(){
		var nominal = parseInt($(this).val());
		if(nominal < 0){
			alert('Nominal Potongan salah!');
			$(this).val('');
			$("#v_potongan_lain").text(toRp(0));
		}else{
			$("#v_potongan_lain").text(toRp(nominal));
		}
	});
	$('#nominal_potongan_dansos').keyup(function(){
		var nominal = parseInt($(this).val());
		if(nominal < 0){
			alert('Nominal Potongan salah!');
			$(this).val('');
			$("#v_potongan_dansos").text(toRp(0));
		}else{
			$("#v_potongan_dansos").text(toRp(nominal));
		}
	});
	...
	$('#plafond').keyup(function(){
		...
		if(nominal <= 0){
			...
			$(this).val('');
			$("#v_plafon").text(toRp(0));
		}else{
			$("#v_plafon").text(toRp(nominal));
		}
	});
	...
	$('#submit_insert').click(function(){
		...
		var potongan_lain = $('#potongan_lain').val();
		var nominal_potongan_lain = $('#nominal_potongan_lain').val();
		var potongan_dansos = $('#potongan_dansos').val();
		var nominal_potongan_dansos = $('#nominal_potongan_dansos').val();
		...
		if(jenis_pinjaman=='' || kode_pinjaman=="" || nama_pinjaman=="" || plafond=="" || tenor=="" || jenis_jasa=="" || status_denda=="" || penarikan_jasa_pinjaman==""){
			alert("Lengkapi Form ..!");
		}else{
			$.ajax({
				...
				data:{
					...
					nominal_potongan_lain : nominal_potongan_lain,
					potongan_dansos : potongan_dansos,
					nominal_potongan_dansos : nominal_potongan_dansos,
					status_pinjamanan : ''
				},
				beforeSend:function(){
					$(".tunggu").show();  
