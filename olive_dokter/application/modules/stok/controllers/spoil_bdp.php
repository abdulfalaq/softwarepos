...
class spoil_bdp extends MY_Controller {
  ...
  {
    $data['konten'] = $this->load->view('spoil_bdp/daftar', NULL, TRUE);
  ...
  {
    $data['konten'] = $this->load->view('spoil_bdp/tambah', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
   public function detail()
  {
    $data['konten'] = $this->load->view('spoil_bdp/detail', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
  public function tambah_spoil()
  {
    $data['konten'] = $this->load->view('spoil_bdp/tambah_spoil', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
  ...
  public function simpan_transaksi_spoil()
  {
   ...
   foreach ($hasil_temp as $temp) {
     ...
     $this->db_master->where('kode_barang',@$temp['kode_bahan']);
     $get_barang_dalam_proses=$this->db_master->get('master_barang_dalam_proses');
     $hasil_barang_dalam_proses=$get_barang_dalam_proses->row();

     $stok_bb['real_stok']=@$hasil_barang_dalam_proses->real_stok - $jumlah_spoil;

     $this->db_master->where('kode_unit_jabung', @$post['kode_unit']);
     $this->db_master->where('kode_barang',@$temp['kode_bahan']);
     $this->db_master->update('master_barang_dalam_proses', $stok_bb);
     

     $this->db->where('kode_unit_jabung', @$post['kode_unit']);
     $get_data_stok=$this->db->get_where('transaksi_stok',array('jenis_transaksi' =>'produksi','kategori_bahan' =>'BDP','sisa_stok >' =>0,'kode_bahan'=>@$temp['kode_bahan'],'status'=>'masuk'));
     ...
     foreach ($hasil_data_stok as $stok) {
       ...
       if($sisa_stok < 0){
        ...
        $record_tstok['kategori_bahan'] = 'BDP';
