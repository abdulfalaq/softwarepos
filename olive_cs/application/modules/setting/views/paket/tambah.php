...
$get_last_id = $this->db_olive->get('opsi_master_paket_temp')->row();
?>
<!-- back button -->
		...
		<div class="col-sm-12">
			<div class="panel panel-default">
				<div class="panel-heading text-right">
					<span class="pull-left" style="font-size: 24px">Tambah Paket </span>
				...
				<div class="panel-body">
					<form id="form" name="form">
							...
							<div class="row">
								<div class="col-md-6">
									<div class="form-group">
										...
										<input  type="text" value="" class="form-control" placeholder="Kode Paket" name="kode_paket" id="kode_paket" required="" onchange="cek_kode_paket()" />
									</div>
								</div>
								<div class=" form-group col-md-6">
									<div class="form-group">
										<label class="gedhi">Nama Paket</label>
										<input type="text" class="form-control" placeholder="Nama paket" value="" name="nama_paket" id="nama_paket" required=""/>
									</div>
								</div>
								<div class="">
									<div class="form-group  col-md-6">
										...
										<div class="input-group">
											...
											<input required type="number" class="form-control input-group" onkeyup="get_nominal_harga_paket()" name="harga_jual" id="harga_jual" value="" required="" />
										</div>
									</div>
								</div>
								<div class="form-group col-md-6">
								...
								<div class="col-md-6">
									<div class="form-group">
										...
										<input type="text" class="form-control"  placeholder="HPP" value="" name="hpp" id="hpp"  readonly="true"/>
									</div>
								</div>
								<div class="form-group col-md-6">
									<label>Status</label>
						...
						<div id="tombol" style="float: right; margin-top: 20px">
							<div class="col-md-12">
								<a class="btn btn-lg btn-info pull-right opsi_stok_out"  id="simpang" onclick="simpan_besar()"><i class="fa fa-send"></i> Simpan</a>
							</div>
						</div>
					</form>
				</div>
...
<div id="modal_simpan_paket" class="modal fade" tabindex="-1" role="dialog" aria-labelledby="myModalLabel4" aria-hidden="true">
...
<script type="text/javascript">
	...
	function hapus_data() {
		var id = $('#id').val();
		$.ajax({
			url: '<?php echo base_url('setting/paket/hapus_data'); ?>',
			type: 'post',
			data:{id:id},
			beforeSend:function(){
				$(".tunggu").show();
	...
	function actEdit(id) {
		...
		var url = "<?php echo base_url().'setting/paket/get_temp_paket'; ?>";
		$.ajax({
			type: 'POST',
			url: url,
			dataType: 'json',
	...
	function add() {  
		...
		if (hpp_total!='' && qty!='') {
			$.ajax( {  
				type :"post",  
				url : "<?php echo base_url() . 'setting/paket/add' ?>",  
				cache :false,  
	...
	function load_tabel(obj){
		$.ajax( {  
			type :"post",  
			url : "<?php echo base_url() . 'setting/paket/load_tabel' ?>",  
			data:{kode_paket:obj},
			success : function(data) { 
	...
	function update() {  
		...
		$.ajax( {  
			type :"post",  
			url : "<?php echo base_url() . 'setting/paket/update' ?>",  
			cache :false,  
