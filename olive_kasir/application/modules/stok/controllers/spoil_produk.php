...
class spoil_produk extends MY_Controller {
  ...
  {
    $data['konten'] = $this->load->view('spoil_produk/daftar', NULL, TRUE);
  ...
  {
    $data['konten'] = $this->load->view('spoil_produk/tambah', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
   public function detail()
  {
    $data['konten'] = $this->load->view('spoil_produk/detail', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
  public function tambah_spoil()
  {
    $data['konten'] = $this->load->view('spoil_produk/tambah_spoil', NULL, TRUE);
    $this->load->view ('admin/main', $data);
  }
  ...
  public function simpan_transaksi_spoil()
  {
   ...
   $this->db->where('jenis_bahan', 'Produk');
   ...
   foreach ($hasil_temp as $temp) {
     ...
     $this->db_master->where('kode_produk',@$temp['kode_bahan']);
     $get_produk=$this->db_master->get('master_produk');
     $hasil_produk=$get_produk->row();

     $stok_bb['real_stok']=@$hasil_produk->real_stok - $jumlah_spoil;

     $this->db_master->where('kode_unit_jabung', @$post['kode_unit']);
     $this->db_master->where('kode_produk',@$temp['kode_bahan']);
     $this->db_master->update('master_produk', $stok_bb);
     

     $this->db->where('kode_unit_jabung', @$post['kode_unit']);
     $get_data_stok=$this->db->get_where('transaksi_stok',array('jenis_transaksi' =>'produksi','kategori_bahan' =>'Produk','sisa_stok >' =>0,'kode_bahan'=>@$temp['kode_bahan'],'status'=>'masuk'));
