...
$this->db->join('clouoid1_olive_master.master_karyawan', 'clouoid1_olive_kasir.opsi_transaksi_layanan.kode_terapis = clouoid1_olive_master.master_karyawan.kode_karyawan', 'left');
...
foreach ($get_temp as $temp) {
	if($temp->jenis_item!='kartu member'){
		...
		<tr>
			...
			<td><?php echo @get_edit_delete_poin($temp->id);?></td>
		</tr>
		<?php
	}else{
		?>
