		...
		<div class="col-sm-12">			
				...
				<div class="row" style="margin-right: 10px">
					...
					<div class="col-md-7">
						<span class="jenis_member">
							<select  class="form-control select2" id="kode_member" name="kode_member" onchange="get_data_member()">
								...
								foreach ($get_member as $member) {
									?>
									<option  value="<?php echo @$member->kode_member;?>"><?php echo @$member->kode_member.' - '.@$member->nama_member;?></option>
									<?php
								}
								?>
							</select>
					...
					<div class="col-md-3" style="padding: 0">
						<div id="lock" class="btn btn-md btn-danger btn-no-radius btn-shadow" onclick="lock_member()" style="width: 50%">LOCK</div>
						...
						<button id="join_member" onclick="join_member();" class="btn btn-md btn-warning btn-no-radius btn-shadow" style="width: 100%;margin-top: 10px">JOIN MEMBER</button>
						<button id="cancel_join_member" onclick="cancel_join_member();" class="btn btn-md btn-danger btn-no-radius btn-shadow" style="width: 100%;margin-top: 10px">CANCEL JOIN</button>
					</div>
				</div>
			</form><hr><hr>
			...
			<div id="tabel1">
				<table id="" class="table table-striped table-bordered table-advance table-hover">
					<tbody>
						<tr>
							<input type="text" name="kode_transaksi" id="kode_transaksi" value="<?php echo 'PEN_'.date('YmdHis');?>" hidden/>
							...
							<td width="20%" style="background-color:#229fcd;">
								<div class="input-icon right" id="form_diskon_item">								
									<input type="number" name="harga" id="harga"  class="form-control data_kasir" placeholder="Harga" onkeyup="cek_harga()" onclick="cek_harga()" readonly>
								</div>
							</td>
							...
							<td width="7%" style="background-color:#229fcd;" >
								<div id="add" onclick="simpan_pesanan_temp()" class="btn purple data_kasir">Add</div>
								<div id="update" onclick="update_pesanan_temp()" class="btn purple data_kasir">Update</div>
							</td>
						</tr>
					</form>
				</tbody>
...
<script type="text/javascript">
	...
	function load_table_temp() {
		...
		$('#tb_pesan_temp').load('<?php echo base_url('kasir/table_pesan_temp'); ?>/'+kode_transaksi);
		get_total_pesanan();
	}
	function join_member() {
		...
		if(kode_member==''){
			...
			$.ajax({
				url: '<?php echo base_url('kasir/simpan_kartu_member'); ?>',
				type: 'post',
	...
	function cancel_join_member() {
		var kode_transaksi=$('#kode_transaksi').val();
		var kode_member=$('#kode_member').val();
		$.ajax({
			url: '<?php echo base_url('kasir/delete_kartu_member'); ?>',
			type: 'post',
			data:{kode_transaksi:kode_transaksi,kode_member:kode_member},
			success: function(hasil){
				$('#join_member').show();
	...
	function actPoin(id) {
		...
		$('#data_poin_member').load('<?php echo base_url('kasir/tabel_transaksi_poin'); ?>/'+id);
	...
	function gunakan_poin(id,obj) {
		...
		var kode_transaksi=$('#kode_transaksi').val();
		$.ajax({
			url: '<?php echo base_url('kasir/gunakan_poin_member'); ?>',
			type: 'post',
	...
	function simpan_pesanan_temp() {
		...
		$.ajax({
			url: '<?php echo base_url('kasir/simpan_pesanan_temp'); ?>',
			type: 'post',
	...
	function actDelete(key) {
		$.ajax({
			url: '<?php echo base_url('kasir/hapus_temp'); ?>',
			type: 'post',
			data:{id:key},
			success: function(hasil){
				load_table_temp();
	...
	function actEdit(key) {
		$.ajax({
			url: '<?php echo base_url('kasir/get_data_temp'); ?>',
			type: 'post',
			data:{id:key},
			dataType:'json',
			success: function(hasil){
	...
	function update_pesanan_temp() {
		...
		var jenis_item=$('#jenis_item').val();
		$.ajax({
			url: '<?php echo base_url('kasir/update_pesanan_temp'); ?>',
			type: 'post',
	...
	function get_total_pesanan() {
		...
		$.ajax({
			url: '<?php echo base_url('kasir/get_total_pesanan'); ?>',
			type: 'post',
	...
	function simpan_transaksi() {
		...
		if(total_pesanan==''){
			...
			$.ajax({
				url: '<?php echo base_url('kasir/simpan_transaksi'); ?>',
				type: 'post',
				data:{
